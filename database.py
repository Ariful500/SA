...
# ══════════════════════════════════════════════
#  OVERAGE LOG (লিমিটের বেশি SMS নিলে যে রেকর্ড থাকবে)
# ══════════════════════════════════════════════

OVERAGE_FILE = "overage_log.json"
_overage_lock = threading.Lock()


def _load_overage() -> dict:
    if not os.path.exists(OVERAGE_FILE):
        return {}
    try:
        with open(OVERAGE_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}


def _save_overage(data: dict, git_message: str = "⚠️ Overage log updated"):
    with _overage_lock:
        with open(OVERAGE_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    try:
        subprocess.run(["git", "config", "user.name", "github-actions[bot]"], check=False)
        subprocess.run(["git", "config", "user.email", "github-actions[bot]@users.noreply.github.com"], check=False)
        subprocess.run(["git", "pull", "origin", "main", "--rebase"], check=False)
        subprocess.run(["git", "add", OVERAGE_FILE], check=False)
        result = subprocess.run(["git", "diff", "--staged", "--quiet"], capture_output=True)
        if result.returncode != 0:
            subprocess.run(["git", "commit", "-m", git_message], check=False)
            subprocess.run(["git", "push", "origin", "main"], check=False)
            print(f"[Overage] ✅ Git saved: {git_message}")
    except Exception as e:
        print(f"[Overage] Git save error: {e}")


async def log_overage(lamix_username: str, range_name: str):
    """একজন ইউজার একটা রেঞ্জে লিমিটের বেশি SMS নিলে কাউন্ট বাড়ায়।"""
    def _do():
        data = _load_overage()
        data.setdefault(lamix_username, {})
        data[lamix_username][range_name] = data[lamix_username].get(range_name, 0) + 1
        _save_overage(
            data,
            f"⚠️ Overage: {lamix_username} / {range_name} = {data[lamix_username][range_name]}",
        )
    await asyncio.to_thread(_do)
